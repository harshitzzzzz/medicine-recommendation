...
def get_predicted_value(patient_symptoms):
    ...
    if len(input_vector) != svc.n_features_in_:
        raise ValueError(f"Input vector has {len(input_vector)} features, but the model expects {svc.n_features_in_}.")
...
def predict():
    if request.method == 'POST':
        ...
        if not symptoms.strip():
            message = "Please provide symptoms or check for spelling errors."
